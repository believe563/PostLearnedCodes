...
public class MarkerActivity extends Activity implements OnMarkerClickListener,
	...
	private void setUpMap() {
		...
		aMap.setInfoWindowAdapter(this);// 设置自定义InfoWindow样式--getInfoWindow和getInfoContent
	...
	private void addMarkersToMap() {
		//文字显示标注，可以设置显示内容，位置，字体大小颜色，背景色旋转角度，长方形的一个像textview
		...
		marker.setPositionByPixels(400, 400);//如果没有这句就没有下面的小气球，只有上面的窗口
		marker.showInfoWindow();// 设置默认显示一个infowinfow，如果没设置第一次不显示上面的窗口，点击才显示
		
		markerOption = new MarkerOptions();//标记图标，没有position不显示
		markerOption.position(Constants.XIAN);//设置为西安市经纬度，如果没有设置这个则不显示
		markerOption.title("西安市").snippet("西安市：34.341568, 108.940174");//snippet：片段显示
		...
		MarkerOptions markerOption1 = new MarkerOptions().anchor(0.5f, 0.5f)
				...
				.snippet("成都市:30.679879, 104.064855").icons(giflist)//显示的是三种颜色的动态气泡
			 .draggable(true).period(50);//period是设置气泡颜色变换的时长
		ArrayList<MarkerOptions> markerOptionlst = new ArrayList<MarkerOptions>();
		markerOptionlst.add(markerOption);
		markerOptionlst.add(markerOption1);
		List<Marker> markerlst = aMap.addMarkers(markerOptionlst, true);//option需要添加到marker中
	...
	public void jumpPoint(final Marker marker) {
		...
		final long start = SystemClock.uptimeMillis();//正常运行时间
		Projection proj = aMap.getProjection();
		Point startPoint = proj.toScreenLocation(Constants.XIAN);
		startPoint.offset(0, -100);//把x,y坐标偏移一下
		final LatLng startLatLng = proj.fromScreenLocation(startPoint);
		final long duration = 1500;

		final Interpolator interpolator = new BounceInterpolator();//篡改时间，可以让它呈现加减速等效果
		handler.post(new Runnable() {
			@Override
			public void run() {
				long elapsed = SystemClock.uptimeMillis() - start;
//				Toast.makeText(MarkerActivity.this, "距离开始时的时间为：" + elapsed, Toast.LENGTH_SHORT).show();
				float t = interpolator.getInterpolation((float) elapsed
						/ duration);
//				Toast.makeText(MarkerActivity.this, "间隔时间为：" + t, Toast.LENGTH_SHORT).show();
				double lng = t * Constants.XIAN.longitude + (1 - t)
						* startLatLng.longitude;
				double lat = t * Constants.XIAN.latitude + (1 - t)
						* startLatLng.latitude;
//				Toast.makeText(MarkerActivity.this, "经纬度为：" + lng+","+lat, Toast.LENGTH_LONG).show();
	...
	public void onMapLoaded() {
		...
		aMap.moveCamera(CameraUpdateFactory.newLatLngBounds(bounds, 150));//moveCamera就是把地图的视角变换到指定的位置
	}

	/**
	 * 监听自定义infowindow窗口的infocontents事件回调--adapter
	 */
	@Override
	...
	/**
	 * 监听自定义infowindow窗口的infowindow事件回调---adapter
	 */
	@Override
